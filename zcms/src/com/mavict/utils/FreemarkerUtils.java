...
import com.mavict.setting.SettingEnumConverter;
...
public final class FreemarkerUtils {
	...
	static {
		convertUtils = new ConvertUtilsBean() {
			...
			public String convert(Object value) {
				if (value != null) {
					...
					if (type.isEnum() && super.lookup(type) == null) {
						super.register(new SettingEnumConverter(type), type);
					} else if (type.isArray() && type.getComponentType().isEnum()) {
						if (super.lookup(type) == null) {
							ArrayConverter arrayConverter = new ArrayConverter(type, new SettingEnumConverter(type.getComponentType()), 0);
			...
			public Object convert(String value, Class clazz) {
				if (clazz.isEnum() && super.lookup(clazz) == null) {
					super.register(new SettingEnumConverter(clazz), clazz);
				}
			...
			public Object convert(String[] values, Class clazz) {
				if (clazz.isArray() && clazz.getComponentType().isEnum() && super.lookup(clazz.getComponentType()) == null) {
					super.register(new SettingEnumConverter(clazz.getComponentType()), clazz.getComponentType());
				}
			...
			public Object convert(Object value, Class targetType) {
				if (super.lookup(targetType) == null) {
					if (targetType.isEnum()) {
						super.register(new SettingEnumConverter(targetType), targetType);
					} else if (targetType.isArray() && targetType.getComponentType().isEnum()) {
						ArrayConverter arrayConverter = new ArrayConverter(targetType, new SettingEnumConverter(targetType.getComponentType()), 0);
						arrayConverter.setOnlyFirstToString(false);
		...
		dateConverter.setPatterns(SettingAttributes.DATE_PATTERNS);
